...
import { createShellCorpusOrgan } from "../src/corpus-organ.js";
...
describe("ShellCorpusOrgan", () => {
	it("has kind=corpus, name=shell, and 1 tool", () => {
		const organ = createShellCorpusOrgan({ cwd: process.cwd() });
	...
	it("applies commandPrefix", async () => {
		const { nerve, corpus } = makeNerve();
		const organ = createShellCorpusOrgan({ cwd: process.cwd(), commandPrefix: "export MYVAR=prefixed" });
		const unmount = organ.mount(corpus);
