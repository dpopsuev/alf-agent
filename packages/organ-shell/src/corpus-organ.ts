...
export interface ShellCorpusOrganOptions {
...
async function handleExec(motor: MotorEvent, nerve: CorpusNerve, opts: ShellCorpusOrganOptions): Promise<void> {
 ...
 * @example
 * ```typescript
 * import { createShellCorpusOrgan } from "@dpopsuev/alef-organ-shell";
 * import { Corpus } from "@dpopsuev/alef-corpus";
 *
 * const corpus = new Corpus();
 * corpus.load(createShellCorpusOrgan({ cwd: process.cwd() }));
 * ```
 */
export function createShellCorpusOrgan(options: ShellCorpusOrganOptions): CorpusOrgan {
	return {
		kind: "corpus",
		name: "shell",
