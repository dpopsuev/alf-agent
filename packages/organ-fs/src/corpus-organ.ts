...
export interface FsCorpusOrganOptions {
...
async function handleRead(motor: MotorEvent, nerve: CorpusNerve, opts: FsCorpusOrganOptions): Promise<void> {
...
async function handleGrep(motor: MotorEvent, nerve: CorpusNerve, opts: FsCorpusOrganOptions): Promise<void> {
	const args = motor.payload;
...
async function handleFind(motor: MotorEvent, nerve: CorpusNerve, opts: FsCorpusOrganOptions): Promise<void> {
	const args = motor.payload;
 ...
 * @example
 * ```typescript
 * import { createFsCorpusOrgan } from "@dpopsuev/alef-organ-fs";
 * import { Corpus } from "@dpopsuev/alef-corpus";
 *
 * const corpus = new Corpus();
 * corpus.load(createFsCorpusOrgan({ cwd: process.cwd() }));
 * ```
 */
export function createFsCorpusOrgan(options: FsCorpusOrganOptions): CorpusOrgan {
	return {
		kind: "corpus",
		name: "fs",
